...
- [ ] **Personnalisation des commandes par projet (mécanisme d'extension)** — comment spécialiser une commande du plugin sans forker toute la commande ni modifier la base. Déclenché par Holoon (2026-07-06, 6 commandes locales fortement personnalisées, en doublon avec `/armature:*`). Analyse à 3 tiers (conventions transverses / extension ciblée par points d'ancrage / override complet assumé), candidate à un ADR. Voir [`command-extension-mechanism.md`](command-extension-mechanism.md).
